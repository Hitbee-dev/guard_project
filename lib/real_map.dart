...
class RealMapState extends State<RealMap> {
  ...
  static final CameraPosition _myhome = CameraPosition(
      target: LatLng(37.50447643875764, 127.04460158259123), zoom: 18);

  static final CameraPosition _mystart = CameraPosition(
      target: LatLng(37.50447643875764, 127.04460158259123), zoom: 14);
            ...
            child: Column(
              children: <Widget>[
                FloatingActionButton(
                  ...
                  backgroundColor: Color(0XFF242959),
                  child:
                      const Icon(Icons.home, color: Colors.white, size: 36.0),
                ),
  ...
  void _onAddMarkerButtonPressed() {
    ...
    if (selected == 0) {
      for (count = 1; count < 138; count++) {
        listMarkerIds.add(markerids[count]);
        setState(() {
          markers[markerids[count]] = Marker(
              markerId: markerids[count],
              position: LatLng(lats[count], lngs[count]),
              infoWindow:
                  InfoWindow(title: titles[count], snippet: address[count]));
        });
      }
      count = 0;
      selected = 1;
    } else {
      for (count = 0; count < 138; count++) {
        setState(() {
          markers[markerids[count]] = Marker(
